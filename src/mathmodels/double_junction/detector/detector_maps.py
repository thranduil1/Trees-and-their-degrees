...
def linf_distance_point_to_segment(point, start, end):
    ...
    if point.shape != start.shape or point.shape != end.shape:
        raise ValueError("point, start, and end must have the same shape.")
    ...
    slopes = np.concatenate([direction, -direction])

    intercepts = np.concatenate([offset, -offset])
    ...
    for i in range(len(slopes)):
        for j in range(i + 1, len(slopes)):
            ...
            s = (intercepts[j] - intercepts[i]) / denominator
    ...
    for s in candidates:
        ...
        distances.append(linf_distance(point, segment_point))
...
def cube_radius_to_boundary(center):
    ...
    return float(np.min(np.minimum(center, 1.0 - center)))
...
def forbidden_distance(
    ...
    for name, position in vertices.items():
        if name != junction_name:
            distances.append(linf_distance(center, position))
...
def detector_data(
    ...
    *,
    ...
    N = next(iter(vertices.values())).size
    ...
    ambient_radius = np.min(np.minimum(center, 1.0 - center))

    target_names = [name for name, kind in vertex_type.items() if kind == target_type]
    ...
    for name in target_names:
        ...
        if detector_radius <= 0.0:
            raise ValueError("detector_radius must be positive.")
        ...
        if not (distance_to_center < outer_radius - tie_tolerance):
            continue

        # Choose a cube strictly between the candidate and the nearest
        # obstruction / permitted detector boundary.
        cube_radius = 0.5 * (distance_to_center + outer_radius)

        normalized_position = 0.5 + (position - center) / (2.0 * cube_radius)
        ...
        if np.any(normalized_position <= tie_tolerance):
            continue

        if np.any(normalized_position >= 1.0 - tie_tolerance):
            continue
