...
def plot_detector_slice(
    ...
    *,
    ...
    if fixed_axis not in range(N):
        raise ValueError("fixed_axis must be an x-coordinate index.")

    if varying_axis not in range(N):
        raise ValueError("varying_axis must be an x-coordinate index.")

    if fixed_axis == varying_axis:
        raise ValueError("fixed_axis and varying_axis must differ.")

    if fixed_value not in {0.0, 1.0}:
        raise ValueError("fixed_value must be 0.0 or 1.0.")
    ...
    if title is None:
        title = f"{target_type}-detector on x[{fixed_axis}] = {fixed_value}"
