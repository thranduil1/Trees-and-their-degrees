...
The fundamental reason for this is that it is much harder to obtain a reasonable visualization in higher dimensions.
...
Topological spaces are easier to understand when broken up into simpler spaces with maps between them.
Because the Higman--Thompson groups can be represented as groups of trees, we wanted to understand the topology of spaces of trees.
In this case, there is one building block per type of junction in the trees, 
and the maps between the building blocks are determined by how the junctions attach to each other.
...
We first consider the case of trees with simple junctions.
...
We are interested in embedding these trees into an $N$-dimensional cube,
thereby going from a combinatorial tree to a geometric tree.
How does the equality between the combinatorial trees translate to the geometric context?
...
\begin{figure}
  ...
  \input{figures/timeline.pdf_tex}
...
In the simple junction case and in the double junction case, there is a function \texttt{make\char`_combinatorial\char`_tree} which holds the combinatorial information about the tree,
and a function \texttt{make\char`_geometric\char`_tree\char`_at} which to a point in $[0,1]^{N+1}$ and the combinatorial information of the tree
...
It is tempting to say that the degree of this junction is $ab-1$ because of the middle picture in figure \ref{timeline}.
However, the equivalent trees at $t=0$ and $t=1$ have a very different structure, 
and it is not immediately clear why their degree should be $ab-1$.

Let's give another definition of the degree by going back to the case of a simple junction.
We represent the junction living inside a small neighborhood, depicted as a circle around it in figure \ref{simplej}.
...
In the simple junction case, the detector takes the embedding of an $a$-junction inside a cube at a point $p$
and records the position of the edge closest to the center of the cube.
We adapt this to the double case to create a detector for $a$-junctions, and one for $b$-junctions,
...
\begin{mdframed}[backgroundcolor=purple!10]
  \centering 
  The detector map that we define in the program is actually a composition of \texttt{make\char`_geometric\char`_tree\char`_at} and the aforementioned procedure:\\
\begin{itemize}
...
\begin{figure}
  ...
  \input{figures/spheresandcubes.pdf_tex}
...
\begin{exemple}
  ...
  \begin{figure}
    ...
    \input{figures/queldegre.pdf_tex}
...
\begin{mdframed}[backgroundcolor=purple!10]
  In the program, there is a function \texttt{degree\char`_on\char`_cube\char`_map} 
  which computes the degree of any map from a cube to a cube which sends the boundary to the boundary.
  The function \texttt{tree\char`_detector\char`_boundary\char`_degree} computes the degree of the $a$- and $b$-detectors.
\end{mdframed}
...
We perform a bounding box test in step 3: for the cell $C$ with set of corners $V$, 
...
We therefore look at the outputs of the detector map,
and pick a regular value that lives inside a cluster of other regular values, to make sure it is in a very regular zone.

\section{Results}

In the simple junction case, we can recover the fact that the degree of the $a$-junction is $a-1$.
In the double junction case, the degree of the $a$-detector is $-(b-1)$ and the degree of the $b$-detector is $a-1$.
...
We could also use more sophisticated programs to find roots.

\subsection{Other problems}
The parameters \texttt{junction\char`_span\char`_a, junction\char`_span\char`_b},\\ 
\texttt{junction\char`_time\char`_gap} 
and \texttt{detector\char`_radius} are crucial to set correctly. 
This is because the actual detector map constructs an infinitesimal detector cube,
but as we cannot do that we need to approximate it by a small enough cube.
The values we have defined work here but they would not work for larger values of a and b.
We could set a formula for these parameters which depends on a and b. 
