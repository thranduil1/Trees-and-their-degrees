...
Let's assume we have an $ab$-junction. 
...
The trees are oriented from leaves to root.}
\label{neighborhood}
\end{figure}
...
Recall the detector map that goes from $[0,1]^{N+1}$ to $[0,1]^N/\partial [0,1]^N$.
If we restrict to the boundary of $[0,1]^{N+1}$, we get a map from something which is equivalent to an $N$-sphere 
to another space which is equivalent to an $N$-sphere.
This is why the notion of degree of maps between spheres applies in this case.

\subsection{Algorithm}
Let us now go through the idea of the algorithm.
Given a regular value $y$, the algorithm looks for the preimages of $y$ in the source.
To do this, it cuts up the source into many small little cubes and looks for the preimage inside each cube.
Of course, searching for the preimage inside every cube would take too much time, so many of them are discarded.


\subsection{Picking a regular value}
